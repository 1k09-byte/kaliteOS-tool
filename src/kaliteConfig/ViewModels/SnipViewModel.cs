...
public sealed partial class SnipViewModel : ObservableObject
{
    ...
    [RelayCommand]
    public async Task ForceRemoveSnippingToolAsync()
    {
        try
        {
            var cmd = @"takeown /f ""C:\Windows\System32\SnippingTool.exe"" /a
icacls ""C:\Windows\System32\SnippingTool.exe"" /grant administrators:F
ren ""C:\Windows\System32\SnippingTool.exe"" ""SnippingTool.bak""
reg add ""HKCU\Control Panel\Keyboard"" /v PrintScreenKeyForSnippingEnabled /t REG_DWORD /d 0 /f
reg add ""HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\SnippingTool.exe"" /v Debugger /t REG_SZ /d ""systray.exe"" /f
powershell -Command ""Get-AppxPackage -AllUsers Microsoft.ScreenSketch | Remove-AppxPackage -AllUsers""
";
            string tempScript = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "remove_snipping_tool.bat");
            System.IO.File.WriteAllText(tempScript, cmd);

            var psi = new System.Diagnostics.ProcessStartInfo("cmd.exe", $"/c \"{tempScript}\"")
            {
            ...
            OnPropertyChanged(nameof(StatusBarHijackText));
            var app = (App)Microsoft.UI.Xaml.Application.Current;
            app.Sniper?.RebindHotkey(SnipSettingsService.Load().HotkeyIndex);
            RefreshHotkeyStatus();
            
            NotifyMessage("Snipping Tool removed", "Built-in Snipping Tool has been completely removed and disabled.");
        }
        catch (Exception ex)
        {
            NotifyMessage("Removal failed", ex.Message);
        }
    }
